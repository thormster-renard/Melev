/*
** my_str_to_wordtab.c for Melev in /home/thormster/Dev/my_str_to_wordtab
** 
** Made by thormster
** Login   <thormster@localhost>
** 
** Started on  Tue Dec 31 00:07:21 2019 thormster
** Last update Wed Jan  1 14:34:25 2020 thormster
*/
...
char		**my_str_to_wordtab(char *s, char delim)
{
  ...
  while (s[i])
    {
      k = 0;
      if ((tab[j] = malloc((word_len(s, i, delim) + 1) * sizeof(char))) == NULL)
	return (NULL);
      while (s[i] && s[i] != delim)
	tab[j][k++] = s[i++];
      while (s[i] == delim)
